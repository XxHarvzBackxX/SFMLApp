...
internal class Util
{
    ...
    public static Vector3f MutateX(Vector3f point, float newX) => new Vector3f(newX, point.Y, point.Z);

    public static Vector3f MutateY(Vector3f point, float newY) => new Vector3f(point.X, newY, point.Z);

    public static Vector3f MutateZ(Vector3f point, float newZ) => new Vector3f(point.X, point.Y, newZ);

    public static Vector3f IncrementMutateX(Vector3f point, float incX) => new Vector3f(point.X + incX, point.Y, point.Z);

    public static Vector3f IncrementMutateY(Vector3f point, float incY) => new Vector3f(point.X, point.Y + incY, point.Z);

    public static Vector3f IncrementMutateZ(Vector3f point, float incZ) => new Vector3f(point.X, point.Y, point.Z + incZ);
}
